...
class PartyCog(commands.Cog):
    ...
    async def view(self, interaction: discord.Interaction, event_id: int):
        async with async_session() as session:
            ...
            unassigned = [s for s in accepted if s.party_number is None]
            by_party: dict[int, list[Signup]] = {}
            for s in accepted:
                if s.party_number is not None:
            ...
            for party_num in sorted(by_party.keys()):
                members = by_party[party_num]
                lines = [f"<@{s.user_id}> — {s.assigned_role or s.requested_role}" for s in members]
                embed.add_field(
                    name=f"Party {party_num} ({len(members)}/{PARTY_CAPACITY})",
                    value="\n".join(lines) or "Empty",
                    inline=False,
                )
            if unassigned:
                lines = [f"<@{s.user_id}> — {s.requested_role}" for s in unassigned]
                embed.add_field(name=f"⚠️ Accepted but unassigned ({len(unassigned)})", value="\n".join(lines), inline=False)
            if not accepted:
